...
beed.boot <- function(data, x, d = 3, est = c("ht", "md", "cfg", "pick"),
  ...
  for (i in 1:nboot)
  {
    ...
    bootsamp <- data[indx, ]
    mod <- beed(
      data = bootsamp, x = x, d = d, est = est, k = k,
      plot = FALSE, margin = margin
    )
    b <- mod$beta
    beta <- cbind(beta, b)
...
beed.confband <- function(data, x, d = 3, est = c("ht", "md", "cfg", "pick"),
  ...
  A.up.beta <- beed(
    data = data, x = x, d = d, est = est, k = k,
    beta = up.beta, margin = margin
  )$A
  A.low.beta <- beed(
    data = data, x = x, d = d, est = est, k = k,
    beta = low.beta, margin = margin
  )$A
  ...
  out <- list(
    A = A.tilde, A.up.beta = A.up.beta, A.low.beta = A.low.beta,
