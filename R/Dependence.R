index.ExtDep <- function(object, model, par, x, u) {
  ...
  if (!any(object == objects)) {
    stop("Wrong object specified")
  }
  ...
  if (object == "pickands") {
    ...
    if (d != length(x)) {
      stop("wrong length of 'x'")
    }
    if (!any(d == c(2, 3))) {
      stop("Pickands dependence function available in 2 or 3 dimensions only.")
    }
  ...
  if (object == "upper.tail") {
    if (d != 2) {
      stop("upper tail dependence coefficient only available in dimension 2")
    }
  ...
  if (object == "lower.tail") {
    ...
    if (model == "EST") {
      if (length(par) != 4) {
        stop("Length of 'par' should be 4.")
      }
    ...
    if (model == "ET") {
      if (length(par) != 2) {
        stop("Length of 'par' should be 2.")
      }
    ...
    if (model == "SN") {
      if (length(par) != 3) {
        stop("Length of 'par' should be 3.")
      }
      corr <- par[1]
      shape <- par[2:3]
      if (missing(u)) {
        stop("'u' needs to be provided.")
      }
...
chi.bsn <- function(u, corr = 0, shape = rep(0, 2), tail = "upper") {
  chibsnup <- function(u, scale = diag(2), shape = rep(0, 2)) {
    res <- double(1)
    x <- c(
      qsn(u, omega = scale[1, 1], alpha = shape[1]),
