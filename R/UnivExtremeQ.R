...
ExtQ <- function(P = NULL, method = "Frequentist", pU = NULL,
  ...
  if (!any(method == methods)) {
    stop("method wrongly specified")
  }

  if (is.null(P)) {
    stop("Must specify the probability associated with the quantile(s)")
  }
  if (is.null(cov)) {
    cov <- as.matrix(c(1))
  }
  ...
  if (method == "Frequentist") {
    ...
    if (missing(param_post)) {
      stop("Matrix of posterior parameters needs to be provided")
    }
    if (ncol(param_post) != (ncol(cov) + 2)) {
      stop("Wrong dimension of param_post")
    }
