dim_ExtDep <- function(model, par = NULL, dim = NULL) {
  ...
  if (is.null(par) && is.null(dim)) {
    stop("par or dim needs to be specified")
  }
  ...
  if (!is.null(par) && is.null(dim)) {
    ...
    if (model == "HR") {
      ...
      if (length(par) != (2^(dim - 1) * (dim + 2) - (2 * dim + 1))) {
        stop("Incorrect parameter dimension for model specified")
      }
    }
