plot.ExtDep_npBayes <- function(x, type, summary.mcmc, burn, y, probs, 
  ...
  if(type == "returns"){
    ...
    Plot.Qset(mcmc=x, nsim=nsim, burn=burn, kn=kn, summary.mcmc=summary.mcmc,
...
plot.ExtDep_npFreq <- function(x, type, est.out, mar1, mar2, dep, P, CEX=1.5, 
  ...
  if(type == "A"){
    ...
    Plot.Qset.F(obj=x, mar1=mar1, mar2=mar2, dep=dep, est.out=est.out, P=P,   
...
plot.ExtDep_npEmp <- function(x, type, est.out, mar1, mar2, dep, P, CEX=1.5, 
  ...
  if(type == "psi"){
    ...
    Plot.Qset.E(obj=x, mar1=mar1, mar2=mar2, dep=dep, est.out=est.out, P=P,   
                xlim=xlim, ylim=ylim, xlab=xlab, ylab=ylab, 
                col.data=col.data, col.Qfull=col.Qfull, CEX=1.5, data=data, ...)
  }else{
    stop("Wrong `type` specified.")
...
plot_A_F <- function(obj, CEX=1.5, ...){
  ...
  plot(obj$w, obj$Ahat$A, type="l", xlim=c(0,1), 
...
plot_h_F <- function(obj, CEX=1.5, ...){ 
  ...
  hist(obj$extdata[,2]/rowSums(obj$extdata), prob=TRUE, 
...
plot_psi <- function(obj, CEX=1.5, ...){
  ...
  plot(obj$psi_hat, type="l", xlim=c(0,pi/2), 
...
plot_h_E <- function(obj, CEX=1.5, ...){ 
  ...
  plot(obj$h_hat, type="l", xlim=c(0,1), ylim=ylim, 
