plot.ExtDep_npBayes <- function(x, type, summary.mcmc, burn, y, probs,
  ...
  w <- simplex(d = 2, n = 100, a = 0.0001, b = 0.9999)
  nsim <- x$nsim
  ...
  if (type == "returns") {
    probs <- returns(x = x, summary.mcmc = summary.mcmc, y = y)
    args <- list(...)
    ...
    PriorVSPosterior.k(mcmc = x, nsim = nsim, burn = burn, CEX = CEX, ...)
  } else if (type == "summary") {
    oldpar1 <- par(
      mfrow = c(2, 2), pty = "s", mar = c(4.5, 4.2, 0.25, 0.25),
    ...
    # if(!hasArg(xlim)){xlim <- NULL}
    # if(!hasArg(ylim)){ylim <- NULL}
    # if(!hasArg(xlab)){xlab <- expression(y[1])}
    # if(!hasArg(ylab)){ylab <- expression(y[2])}

    Plot.Qset(
      mcmc = x, nsim = nsim, burn = burn, kn = kn, summary.mcmc = summary.mcmc,
      est.out = est.out, QatCov1 = QatCov1, QatCov2 = QatCov2,
      P = P, mar1 = mar1, mar2 = mar2, dep = dep, CEX = 1.5,
      col.data = col.data, col.Qfull = col.Qfull, col.Qfade = col.Qfade, data = data, ...
    )
  } else {
...
plot.ExtDep_npFreq <- function(x, type, est.out, mar1, mar2, dep, P, CEX = 1.5,
                               col.data, col.Qfull, data = NULL, ...) {
  # x is an object of class ExtDep_npFreq
  ...
  if (type == "A") {
    ...
    oldpar1 <- par(
      mfrow = c(1, 2), pty = "s", mar = c(2, 4.25, 0.25, 0.25),
      oma = c(0, 0, 0, 0), mgp = c(1, 1, 0), cex.axis = CEX
    )
    on.exit(par(oldpar1))
    ...
    Plot.Qset.F(
      obj = x, mar1 = mar1, mar2 = mar2, dep = dep, est.out = est.out, P = P,
      xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab,
      col.data = col.data, col.Qfull = col.Qfull, CEX = 1.5, data = data, ...
    )
  } else {
    stop("Wrong `type` specified.")
  }
...
plot.ExtDep_npEmp <- function(x, type, est.out, mar1, mar2, dep, P, CEX = 1.5,
                              col.data, col.Qfull, data = NULL, ...) {
...
summary.ExtDep_npBayes <- function(object, w, burn, cred = 0.95, plot = FALSE, ...) {
  if (missing(w)) {
    ww <- simplex(d = 2, n = 100, a = 0.0001, b = 0.9999)
  } else {
    if (is.matrix(w)) {
      if (ncol(w) != 2 | any(rowSums(w) != 1)) {
        stop("Argument `w` is ill defined. If in matrix form, it should have 2 columns and each row summing to 1.")
      } else {
        ww <- w
      }
    } else if (is.vector(w)) {
      if (any(w > 1)) {
        stop("Some component(s) of `w` are greater than 1.")
      } else {
        ww <- cbind(w, 1 - w)
      }
    } else {
      stop("Wrong format for `w`.")
    }
  }
  ...
  # posterior k
  qk <- quantile(object$k[burn:nsim], c(alpha, 0.5, 1 - alpha), type = 3)
  k.median <- qk[2]
  k.up <- qk[3]
  k.low <- qk[1]

  bpg <- NULL
  for (i in 1:(max(object$k, na.rm = TRUE) + 1)) bpg[[i]] <- bp(ww, i)
  ...
  if ("mar1" %in% names(object)) {
    mar1_post <- mar2_post <- matrix(NA, iters, ncol(object$mar1)) # Matrix for the marginal parameters
  }

  for (i in burn:nsim) {
    l <- i - burn + 1
    ki <- object$k[i]
    etai <- object$eta[i, 1:(ki + 1)]
    ...
    if ("mar1" %in% names(object)) { # If the marginal parameters have been fitted
      mar1_post[l, ] <- object$mar1[i, ]
      mar2_post[l, ] <- object$mar2[i, ]
    }
  }
  ...
  if ("mar1" %in% names(object)) {
    out <- list(
      k.median = k.median, k.up = k.up, k.low = k.low,
  ...
  if (plot) {
    plot.ExtDep_npBayes(x = object, type = "summary", summary.mcmc = out, burn = burn, ...)
  }
...
##################################################
##################################################
### Internal functions for plot.ExtDep_npBayes ###
##################################################
##################################################
...
fbplot_h <- function(pmcmc, h_true, wgrid, CEX = 1.5) {
  ...
  h_med <- fbplot(t(pmcmc$h_post), wgrid,
    xlim = c(0, 1), ylim = c(0, max(h_true) + .5),
...
PriorVSPosterior.pm <- function(mcmc, nsim, burn, CEX = 1.5, ...) {
  ...
  if (prior.pm == "unif") {
    ...
    # xseq <- seq(from=min(0.0001,a), to=max(0.9999,b), length=100)
    ffun <- function(x) dunif(x, min = a, max = b)
    curve(ffun, from = 0, to = 0.5, ylim = c(0, ydmax + 1), col = "green2", lwd = 2, xlab = "", ylab = "", ...)
  }

  if (prior.pm == "beta") {
    if (length(a) == 1) stop("Check hyperparameters a and b.")
    # xseq <- seq(from=0.0001, to=0.9999, length=100)
    ffun <- function(x) dbeta(x, shape1 = a, shape2 = b)
    curve(ffun, from = 0, to = 0.5, ylim = c(0, ydmax + 1), col = "green2", lwd = 2, xlab = "", ylab = "", ...)
  }

  lines(seq(0, 0.5, length = length(postp0)), postp0, col = "red", lwd = 2)
...
Plot.Qset <- function(mcmc, nsim, burn, kn, summary.mcmc, est.out,
                      ...
                      mar1, mar2, dep = TRUE, CEX = 1.5,
                      col.data, col.Qfull, col.Qfade, data = NULL, ...) {
  args <- list(...)
  if ("xlab" %in% names(args)) {
    xlab <- args$xlab
  } else {
  ...
  if (mar.fit) {
    ...
    if (is.null(QatCov1) && d.cov1 > 1) {
      stop("QatCov1 argument required")
    }
    if (is.null(QatCov2) && d.cov2 > 1) {
      stop("QatCov2 argument required")
    }
    if (!is.null(QatCov1) && (d.cov1 > 1) && (ncol(QatCov1) != (d.cov1 - 1))) {
      stop("Wrong dimensions of QatCov1")
    }
    if (!is.null(QatCov2) && (d.cov2 > 1) && (ncol(QatCov2) != (d.cov2 - 1))) {
      stop("Wrong dimensions of QatCov2")
    }
    ...
    if (n.cov1 != n.cov2) {
      stop("QatCov1 and QatCov2 sould have the same dimensions")
    }
    if (n.cov1 > 3) {
      stop("plot.ExtDep will display maximum 3 covariate levels")
    }
    ...
    if (missing(mar1)) {
      stop("mar1 should be provided when marginal parameters weren't fitted")
    }
    if (missing(mar2)) {
      stop("mar2 should be provided when marginal parameters weren't fitted")
    }
  ...
  if (missing(est.out)) {
    ...
    for (i in 1:npost) {
      nuShat_post[i] <- integrate(Vectorize(function(t) {
        den(t, beta = summary.mcmc$beta_post[[i]], gamma = c(gamhat1_post[i], gamhat2_post[i]))
      }), 0, 1)$value
    }
  ...
  if (dep) {
    ...
    if (is.null(col.data)) {
      if (n.cov1 > 1) {
        Colors <- rbPal(10)
        col.data <- Colors[as.numeric(cut(mcmc$cov1[, 2], breaks = 10))]
      } else {
  ...
  for (i in 1:m) {
    ...
    for (z in 1:n.cov1) {
      ...
      for (j in 1:nw) {
        tmp <- rbind(
          muhat1_post[, z] + sighat1_post / gamhat1_post * ((kn[1] * nuShat_post * Shat_post[j, 1, ] / P[i])^(gamhat1_post) - 1),
          muhat2_post[, z] + sighat2_post / gamhat2_post * ((kn[2] * nuShat_post * Shat_post[j, 2, ] / P[i])^(gamhat2_post) - 1)
        )

        tmp2[j, , ] <- cbind(
          c(tmp[1, which(tmp[1, ] == func(tmp[1, ])[1])[1]], tmp[2, which(tmp[2, ] == func(tmp[2, ])[1])[1]]),
          apply(tmp, 1, mean),
...
#################################################
#################################################
### Internal functions for plot.ExtDep_npFreq ###
#################################################
#################################################
...
plot_h_F <- function(obj, CEX = 1.5, ...) {
  ...
  ylim <- c(0, max(obj$hhat, na.rm = T))

  hist(obj$extdata[, 2] / rowSums(obj$extdata),
    prob = TRUE,
    xlim = c(0, 1), main = "", ylim = ylim, ylab = "", xlab = "", ...
  )

  lines(obj$w, obj$hhat, type = "l", lwd = 2, col = 2)

  points(0, head(obj$p0, 1), pch = 16, cex = 2, col = 2)
  points(1, tail(obj$p1, 1), pch = 16, cex = 2, col = 2)
...
Plot.Qset.F <- function(obj, mar1, mar2, dep = TRUE, est.out = FALSE, P,
                        col.data, col.Qfull, CEX = 1.5, data = NULL, ...) {
  args <- list(...)
  if ("xlab" %in% names(args)) {
    xlab <- args$xlab
  } else {
  ...
  if (mar.fit == FALSE && (is.null(mar1) || is.null(mar2))) {
    stop("missing marginal parameters mar1 and mar2")
  }
  ...
  for (i in 1:m) {
    ...
    Qhat[, , i] <- cbind(
      muhat1 + sighat1 / gamhat1 * ((kn * nuShat * xS / P[i])^gamhat1 - 1),
...
################################################
################################################
### Internal functions for plot.ExtDep_npEmp ###
################################################
################################################
...
plot_psi <- function(obj, CEX = 1.5, ...) {
  ...
  plot(obj$psi_hat,
    type = "l", xlim = c(0, pi / 2),
    ylim = c(0, max(obj$psi_hat[, 2])), ylab = "", xlab = "", lwd = 2, col = 2, , xaxt = "n", ...
  )
...
plot_h_E <- function(obj, CEX = 1.5, ...) {
  ...
  ylim <- c(0, max(obj$h_hat[, 2], na.rm = T))

  plot(obj$h_hat,
    type = "l", xlim = c(0, 1), ylim = ylim,
...
Plot.Qset.E <- function(obj, mar1, mar2, dep = TRUE, est.out = FALSE, P,
  ...
  for (i in 1:m) {
    Qhat[, , i] <- cbind(
      muhat1 + sighat1 / gamhat1 * ((kn1 * nuShat * xS / P[i])^gamhat1 - 1),
...
############################################
############################################
### Hidden functions for computing Qsets ###
############################################
############################################
