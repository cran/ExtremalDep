rExtDep <- function(n, model, par, angular = FALSE, mar = c(1, 1, 1), num,
  ...
  if (model == "semi.bvexceed") {
    ...
    if (length(threshold) != 2) {
      stop("'threshold' should be a bivariate vector")
    }
    if (!(exceed.type) %in% c("and", "or")) {
      stop(" 'exceed.type' must be 'and' or 'or'.")
    }
  }
  ...
  if (!angular) { # modify the marginal distributions
    ...
    if (!all(mar == 1)) {
      ...
      if (is.matrix(mar)) {
        if (ncol(mar) != 3) {
          stop("mar must have 3 columns")
        }
        if (nrow(mar) != d) {
          stop(paste("mar must have ", d, " rows", sep = ""))
        }
      }
...
rh.mixt <- function(N = 1000, beta) {
  ...
  rand.samples <- sapply(
    1:N, function(i, prob, k, U) {
      j <- min(which((U[i] < cumsum(prob))))
      return(rbeta(1, j, k - j))
    },
    prob, k, U
  )
